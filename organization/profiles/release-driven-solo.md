...
pc-setup の current release-driven development で使用する Operating Model を定義します。
...
- NixOS の machine/system state は Nix/Flake/Home Manager が canonical。mise は portable user CLI の frontend として既存責務分担を維持し、Nix の system dependency ownership を奪わない。
- `dotfiles` への handoff と machine provisioning の境界を維持する。
- self-hosted Infisical の endpoint / project metadata と secret-free repository contract を既存実装どおり保持する。
